...
\item \texttt{sort(T{[}{]} a, Comparator\textless{}T\textgreater{} c)}
sorts array \texttt{a}. 
...
or \texttt{new LinkedList\textless{}\textgreater{}()} creates
new list. 
...
\item \texttt{.map(f)} maps \texttt{f} (a method or constructor) to a stream. 
\item \texttt{.filter(f)} filters out elements for which \texttt{f} applied
to the element returns \texttt{false}. 
\item \texttt{.reduce(id, f)} reduces the stream starting from \texttt{id},
...
Static methods: \texttt{ContainingClass::method}; Instance methods:
\texttt{containingObject::method}; Instance method of arbitrary object:
\texttt{ContainingType::method}; Constructors: \texttt{ClassName::new}.
...
B\textgreater{}} where \texttt{A} and \texttt{B} are \emph{type
parameters}. We can then use \texttt{A} and \texttt{B} as normal type
names within that class. 
\item \emph{Methods}: example: \texttt{public static \textless{}S, T\textgreater{}
Pair\textless{}S, T\textgreater{} makePair(s First, T second)}.
Then use \texttt{S} and \texttt{T} freely inside method. 
\end{enumerate}

\paragraph{Wildcards}
\begin{enumerate}
\item \texttt{Set\textless{}Shape\textgreater{}} refers to a set of
shapes. We can add any shape to this set, and can retrieve shapes.
\texttt{Set\textless{}Circle\textgreater{}} however is \emph{not}
a subtype of \texttt{Set\textless{}Shape\textgreater{}}. 
\item \texttt{Set\textless{}? extends Shape\textgreater{}} refers to
any set whose elements are a subclass of \texttt{Shape}. We cannot
add to this set, but we can retrieve shapes. A \texttt{Set\textless{}Circle\textgreater{}}
...
Unchecked exceptions, such as runtime exceptions do not need to be
caught / propogated.
...
Add using recursion. 
\item \emph{Iterators}: Usually implemented as inner classes. Implement
\texttt{Iterable}. Need to be careful of concurrent modifications.
\end{enumerate}
...
\item If has two children, swap with leftmost child of the right child.
\end{enumerate}
\end{enumerate}
